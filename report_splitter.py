...
class DataStore:
    ...
    def _load(self):
        ...
        numbers = self.settings.get("phone_numbers", DEFAULT_CONTACT_NUMBERS)
        if not isinstance(numbers, list):
            numbers = list(DEFAULT_CONTACT_NUMBERS)
        self.settings["phone_numbers"] = [
    ...
    def save_settings(self, values: dict):
        ...
        if values.get("phone_numbers") is not None:
            numbers = values["phone_numbers"]
...
class ReportSplitter(tk.Tk):
    ...
    def _build_ui(self):
        ...
        brand = tk.Frame(hdr, bg=PRIMARY_DARK, width=58, height=58)
        ...
        if self._app_icon is not None:
            ...
            tk.Label(brand, image=self._header_icon, bg=PRIMARY_DARK).pack(
                expand=True)
        tk.Label(hdr, text="Hilton High School", bg=CARD, fg=TEXT,
    ...
    def _build_tab_upload(self):
        ...
        gate = tk.Frame(tab, bg=CARD, pady=10, padx=12,
                        bd=1, relief="solid")
        gate.pack(fill="x")
        tk.Label(gate, text="Website upload password:",
                 bg=CARD, fg=TEXT, font=FONT_BOLD).pack(side="left")
        ...
        self.upload_gate_label = tk.Label(gate, text="Locked",
                                          bg=CARD, fg="#b45309", font=FONT_SM)
    ...
    def _build_history_table(self, parent):
        ...
        self.history_storage_label = tk.Label(
            toolbar, text="", bg=CARD, fg=TEXT_MUTED, font=FONT_SM,
        ...
        tk.Label(website_options, text="Website announcement:",
                 bg=BG, fg=TEXT, font=FONT_BOLD).pack(side="left", padx=(0, 8))
        self.website_mode_var = tk.StringVar(value="image")
        tk.Radiobutton(
            website_options, text="Image banner", variable=self.website_mode_var,
            value="image", command=self._update_website_media_mode,
            bg=BG, fg=TEXT, activebackground=BG,
            selectcolor=PRIMARY_PALE, font=FONT
        ).pack(side="left", padx=3)
        self.website_banner_button = self._b(
            website_options, "Choose Website Banner", self._browse_banner,
            bg="#0369a1", fg="white")
        self.website_banner_button.pack(side="left", padx=(4, 8))
        self.website_banner_status = tk.Label(
            website_options, text="No banner selected",
            bg=BG, fg=TEXT_MUTED, font=FONT_SM)
        self.website_banner_status.pack(side="left", padx=4)

        announcement_row = tk.Frame(parent, bg=BG)
        announcement_row.pack(fill="x", pady=(0, 10))
        tk.Radiobutton(
            announcement_row, text="Text announcement",
            variable=self.website_mode_var, value="text",
            command=self._update_website_media_mode,
            bg=BG, fg=TEXT, activebackground=BG,
            selectcolor=PRIMARY_PALE, font=FONT
        ).pack(side="left", padx=3)
        self.announcement_text_widget = tk.Text(
            announcement_row, height=3, width=72, wrap="word",
            font=FONT, bg=ENTRY_BG, fg=TEXT, bd=1, relief="solid",
            highlightthickness=1, highlightbackground=BORDER)
        self.announcement_text_widget.pack(
            side="left", fill="x", expand=True, padx=(8, 8))
        self._b(announcement_row, "Clear Text", self._clear_announcement,
                bg=BTN_BG).pack(side="left")
        self._update_website_media_mode()
    ...
    def _build_secret_settings(self, tab):
        ...
        contacts = tk.Frame(panel, bg=CARD)
        contacts.pack(fill="x", pady=(12, 4))
        tk.Label(contacts, text="Website contact phone numbers",
                 bg=CARD, fg=TEXT, font=FONT_BOLD).pack(anchor="w")
        tk.Label(
            contacts,
            text="These numbers appear as click-to-call contacts on the public website.",
            bg=CARD, fg=TEXT_MUTED, font=FONT_SM
        ).pack(anchor="w", pady=(0, 6))
        phone_controls = tk.Frame(contacts, bg=CARD)
        phone_controls.pack(fill="x")
        self.secret_phone_var = tk.StringVar()
        tk.Entry(phone_controls, textvariable=self.secret_phone_var,
                 font=FONT, width=28).pack(side="left", padx=(0, 6), ipady=4)
        self._b(phone_controls, "Add", self._add_secret_phone,
                bg=PRIMARY, fg="white").pack(side="left", padx=2)
        self._b(phone_controls, "Edit Selected", self._edit_secret_phone,
                bg=ACCENT, fg="white").pack(side="left", padx=2)
        self._b(phone_controls, "Remove Selected", self._remove_secret_phone,
                bg=RED, fg="white").pack(side="left", padx=2)
        phone_list_frame = tk.Frame(contacts, bg=CARD)
        phone_list_frame.pack(fill="x", pady=(7, 0))
        phone_scroll = ttk.Scrollbar(phone_list_frame, orient="vertical")
        phone_scroll.pack(side="right", fill="y")
        self.secret_phone_list = tk.Listbox(
            phone_list_frame, height=4, font=FONT, bg=ENTRY_BG, fg=TEXT,
            selectbackground=PRIMARY_PALE, selectforeground=TEXT,
            bd=1, relief="solid", highlightthickness=0,
            exportselection=False, yscrollcommand=phone_scroll.set)
        self.secret_phone_list.pack(side="left", fill="x", expand=True)
        phone_scroll.config(command=self.secret_phone_list.yview)
        self.secret_phone_list.bind(
            "<<ListboxSelect>>", self._load_selected_secret_phone)
        self._refresh_secret_phone_list()
    ...
    def _save_secret_settings(self):
        self._store.save_settings({
            **{key: var.get() for key, var in self.secret_vars.items()},
            "phone_numbers": self._secret_phone_values(),
        })
        self.upload_password_var.set("")
        messagebox.showinfo("Saved", "Administrator settings saved.")

    def _secret_phone_values(self):
        if not hasattr(self, "secret_phone_list"):
            return list(self._store.settings.get(
                "phone_numbers", DEFAULT_CONTACT_NUMBERS))
        return [
            self.secret_phone_list.get(index)
            for index in range(self.secret_phone_list.size())
        ]

    def _refresh_secret_phone_list(self):
        if not hasattr(self, "secret_phone_list"):
            return
        self.secret_phone_list.delete(0, "end")
        for number in self._store.settings.get(
                "phone_numbers", DEFAULT_CONTACT_NUMBERS):
            self.secret_phone_list.insert("end", number)

    def _load_selected_secret_phone(self, _event=None):
        selected = self.secret_phone_list.curselection()
        if selected:
            self.secret_phone_var.set(
                self.secret_phone_list.get(selected[0]))

    def _add_secret_phone(self):
        number = self.secret_phone_var.get().strip()
        if len(re.sub(r"\D", "", number)) < 3:
            messagebox.showwarning(
                "Invalid phone number",
                "Enter a phone number containing at least three digits.")
            return
        numbers = self._secret_phone_values()
        numbers.append(number)
        self._store.settings["phone_numbers"] = numbers
        self.secret_phone_var.set("")
        self._refresh_secret_phone_list()

    def _edit_secret_phone(self):
        selected = self.secret_phone_list.curselection()
        number = self.secret_phone_var.get().strip()
        if not selected:
            messagebox.showinfo(
                "Select a number",
                "Select a phone number to edit, then change the value above.")
            return
        if len(re.sub(r"\D", "", number)) < 3:
            messagebox.showwarning(
                "Invalid phone number",
            ...
            return
        numbers = self._secret_phone_values()
        numbers[selected[0]] = number
        self._store.settings["phone_numbers"] = numbers
        self._refresh_secret_phone_list()
        self.secret_phone_list.selection_set(selected[0])

    def _remove_secret_phone(self):
        selected = list(self.secret_phone_list.curselection())
        if not selected:
            messagebox.showinfo(
                "Select a number", "Select one or more phone numbers to remove.")
            return
        if not messagebox.askyesno(
                "Remove phone numbers",
                f"Remove {len(selected)} selected phone number(s)?"):
            return
        numbers = self._secret_phone_values()
...
"""
    ...
    def _website_contact_markup(self):
        numbers = self._store.settings.get(
            "phone_numbers", DEFAULT_CONTACT_NUMBERS)
        items = []
        for value in numbers if isinstance(numbers, list) else []:
            display = str(value).strip()
            if not display or not re.search(r"\d", display):
                continue
            dial = re.sub(r"\D", "", display)
            if display.startswith("+"):
                dial = "+" + dial
            items.append(
                f'<li><a href="tel:{html.escape(dial, quote=True)}">'
                f'{html.escape(display)}</a></li>')
        if not items:
            items.append(
                '<li><span style="color:var(--text-muted)">'
                'No contact numbers have been configured.</span></li>')
        return '<ul class="contact-numbers">\n' + "\n".join(items) + \
            '\n          </ul>'

    def _website_announcement_markup(self):
        if not hasattr(self, "announcement_text_widget") \
                or not hasattr(self, "website_mode_var") \
                or self.website_mode_var.get() != "text":
            return ""
        text = self.announcement_text_widget.get("1.0", "end").strip()
        if not text:
            return ""
        safe_text = html.escape(text, quote=False).replace("\n", "<br />")
        return (
            '    <section id="announcement-section" class="announcement-section" '
            'aria-label="School announcement">\n'
            '      <div class="announcement-box" role="note" '
            'style="background:#fff3cd;border:1px solid #f0c36d;'
            'border-left:6px solid #d39e00;border-radius:12px;'
            'padding:16px 20px;margin:0 auto 24px;max-width:900px;'
            'color:#664d03;box-shadow:0 3px 12px rgba(102,77,3,.12)">\n'
            '        <strong style="display:block;font-size:13px;'
            'letter-spacing:.08em;text-transform:uppercase;margin-bottom:6px">'
            'Announcement</strong>\n'
            f'        <div style="font-size:16px;line-height:1.55">{safe_text}</div>\n'
            '      </div>\n'
            '    </section>'
        )
    ...
    def _compile_site(self):
        ...
        try:
            ...
            if self.website_mode_var.get() == "image" \
                    and self.banner_path and Path(self.banner_path).exists():
            ...
            if index.exists():
                site_html = index.read_text("utf-8")
                site_html = re.sub(
                    r'<ul class="contact-numbers">.*?</ul>',
                    self._website_contact_markup(),
                    site_html, count=1, flags=re.DOTALL)
                announcement = self._website_announcement_markup()
                if announcement:
                    marker = '    <section id="banner-section"'
                    if marker in site_html:
                        site_html = site_html.replace(
                            marker, announcement + "\n\n" + marker, 1)
                index.write_text(site_html, "utf-8")
    ...
    def _browse_banner(self):
        self.website_mode_var.set("image")
        self._update_website_media_mode()
        path = filedialog.askopenfilename(
    ...
    def _update_website_media_mode(self):
        if not hasattr(self, "website_mode_var"):
            return
        use_text = self.website_mode_var.get() == "text"
        if hasattr(self, "announcement_text_widget"):
            self.announcement_text_widget.config(
                state="normal" if use_text else "disabled")
        if hasattr(self, "website_banner_button"):
            self.website_banner_button.config(
                state="disabled" if use_text else "normal")
        if hasattr(self, "website_banner_status"):
            if use_text:
                self.website_banner_status.config(
                    text="Text announcement will be used", fg=AMBER)
            elif self.banner_path:
                self.website_banner_status.config(
                    text=f"Selected: {Path(self.banner_path).name}", fg=PRIMARY)
            else:
                self.website_banner_status.config(
                    text="No banner selected", fg=TEXT_MUTED)

    def _clear_announcement(self):
        if hasattr(self, "announcement_text_widget"):
            self.announcement_text_widget.delete("1.0", "end")
        if hasattr(self, "website_mode_var"):
            self.website_mode_var.set("text")
            self._update_website_media_mode()
    ...
    def _clear_banner(self):
        ...
        if hasattr(self, "website_mode_var"):
            self._update_website_media_mode()
        elif hasattr(self, "website_banner_status"):
            self.website_banner_status.config(
                text="No banner selected", fg=TEXT_MUTED)
